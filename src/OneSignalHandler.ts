...
export const sendAppErrorAlert = async (
  organisation_id: string,
  conversation_id: string,
  ...
  const allSuperAdminEmails = await db
    ...
    .where("user_roles.role", "=", UserRoleType.SUPER_ADMIN)
    .execute();
  const allOrganisationUserEmails = await db
    .selectFrom("users")
    .leftJoin("user_roles", "user_roles.user_id", "users.id")
    .select(["users.email"])
    .where("users.email", "!=", "")
  ...
  const allEmails = new Set(
    allSuperAdminEmails
      .map((e) => e.email)
      .concat(allOrganisationUserEmails.map((e) => e.email))
  );

  if (allEmails.size > 0) {
    const { name: organisation_name } = await db
      .selectFrom("organisations")
      .select(["name"])
    ...
    const email = new OneSignal.Notification();
    email.app_id = "2962b8af-f3e3-4462-989e-bc983ebaf07a";
    email.include_email_tokens = Array.from(allEmails).filter((email) => email);
    email.target_channel = "email";
    email.email_subject = `ALERT - Someone reported an issue with your application`;
    email.template_id = "1e737824-b5e2-4bf9-ba35-f023f1875a64"; // 8ai Application Error Alert
    email.custom_data = constrainCustomDataToSize(
      {
        id: conversation_id,
        url: `https://app.8ai.co.nz/conversations/${conversation_id}`,
        organisation_name,
        contact_details: contact_details || "An anonymous user",
        issue_description,
      },
      context
    );
    ...
    await oneSignal.createNotification(email).then(
      (response) => {
        context.log(response);
        context.log(
          `Successfully sent app error alert alert for ${conversation_id} and organisation ${organisation_name}`
        );
      },
      (error) => context.error(error)
    );
